...
export default class AddonsLoader {
    ...
    static async globalModulesPath(): Promise<string> {
        return exec("npm root -g");
    }

    static async loadAddons(): Promise<[AddonModule[], Error[]]> {
        let addons: AddonModule[] = [];
        let errors: Error[] = [];
    ...
    static async loadAddon(name: string, globalModulesPath: string): Promise<AddonModule> {
