...
describe('NumericCondition.NotEqual', () => {
  let condition: NumericCondition.NotEqual;

  beforeEach(() => {
    condition = new NumericCondition.NotEqual();
  });
  ...
  describe('#evaluate(inputs:)', () => {
    ...
    it('should return `true` when the lhs is Number.MIN_VALUE and rhs is the minimum safe integer', async () => {
      await expect(evaluateInputs(Number.MIN_VALUE, Number.MIN_SAFE_INTEGER)).resolves.toBe(true);
    });

    it('should return `true` when the lhs is the number 0 and rhs is the minimum safe integer', async () => {
      await expect(evaluateInputs(0, Number.MIN_SAFE_INTEGER)).resolves.toBe(true);
    });

    it('should return `true` when the lhs is the number 0 and rhs is the minimum safe integer negated', async () => {
      await expect(evaluateInputs(0, -Number.MIN_SAFE_INTEGER)).resolves.toBe(true);
    });
    ...
    it('should return `true` when the lhs is positive infinity and rhs is negative infinity', async () => {
